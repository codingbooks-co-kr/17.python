...
class Student:			# 학생 클래스
    ...
    def get_total(self):		# 메서드 유형3 (매개변수 X, 반환값 O)
        return sum(self.scores)	# 리스트 안의 모든 점수를 더한 후 반환

student = Student("민수")		# 객체(student) 생성

student.print_info()     		# 메서드 유형1 호출 (이름 출력)
student.add_score(85)     		# 메서드 유형2 호출 (점수 추가)
student.add_score(92)     		# 메서드 유형2 호출 (점수 추가)
student.add_score(78)     		# 메서드 유형2 호출 (점수 추가)
total = student.get_total()		# 메서드 유형3 호출 (점수 합산 출력)
print(f">> 누적 점수: {total}점")